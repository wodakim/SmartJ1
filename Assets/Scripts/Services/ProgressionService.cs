...
namespace EntropySyndicate.Services
{
    public class ProgressionService
    {
        ...
        public ProgressionService(SaveService save, MetaProgressionConfig meta, EconomyService economy, MissionService mission, AnalyticsService analytics)
        {
            _save = save;
            _meta = meta;
            _economy = economy;
        ...
        public bool UpgradeForgeNode(string nodeId)
        {
            for (int i = 0; i < _meta.forgeNodes.Count; i++)
            {
                ...
                if (!_economy.Spend(node.costCurrency, node.costAmount))
                {
                    return false;
                }
