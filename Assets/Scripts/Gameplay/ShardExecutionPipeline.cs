...
namespace EntropySyndicate.Gameplay
{
    public class ShardExecutionPipeline
    {
        ...
        private readonly Collider2D[] _overlapBuffer = new Collider2D[96];
        ...
        public bool TryCast(ShardType type, Vector2 position)
        {
            ...
            string shard = type.ToString();
            _analytics.LogEvent("shard_used", shard);
            _analytics.MarkShardUsage(shard);
            return true;
        }

        private void ApplyEffect(ShardType type, ShardDefinition definition, Vector2 position)
        {
            ContactFilter2D filter = default;
            filter.NoFilter();
            int hitCount = Physics2D.OverlapCircle(position, definition.radius, filter, _overlapBuffer);
            float multiplier = 1f;
            ...
            {
                ChaosEntity entity = _overlapBuffer[i].GetComponent<ChaosEntity>();
                if (entity != null)
                {
